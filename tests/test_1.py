...
class TestComment:
    def test_sms_sender(self, monkeypatch, twilio_client, twilio_sid):
        ...
        assert hasattr(
            homework, "sms_sender"
        ), "Функция `sms_sender()` не существует. Не удаляйте её."
        assert hasattr(
            homework.sms_sender, "__call__"
        ), "Функция `sms_sender()` не существует. Не удаляйте её."
        assert (
            len(signature(homework.sms_sender).parameters) == 1
    ...
    def test_get_status(self, monkeypatch, vk_sid, response_get):
        ...
        assert hasattr(
            homework.get_status, "__call__"
        ), "Функция `get_status()` не существует. Не удаляйте её."
        assert (
            len(signature(homework.get_status).parameters) == 1
